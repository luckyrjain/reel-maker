...
class MetricsFetcher:
    def fetch(self, cut, credential, db) -> EngagementMetrics | None:
        ...
        abort the whole pull. db is passed through for fetchers (YouTube) that
        need to refresh and persist an expired access token."""
...
class YouTubeMetricsFetcher(MetricsFetcher):
    ...
    def fetch(self, cut, credential, db) -> EngagementMetrics | None:
        # Google access tokens expire in ~1h; get_valid_access_token refreshes
        # and persists a new one via the stored refresh_token first if needed
        # — reusing the same helper YouTubePublisher.publish() uses, so this
        # scheduled pull doesn't silently 401 on every credential older than
        # an hour.
