...
# Tests
.venv/bin/pytest                            # 691 tests across 30+ files, default run (4 test_compositor tests need ffmpeg
...
worker/
  ...
  tasks/
    ...
    render.py         render_cut(job_id) — prepare (refuses an already-posted cut), resolve_or_reuse,
                      ...
                      policy; sets cut.rendered_pins_fingerprint = compute_pins_fingerprint_for_render(
                      db, cut.id) — never the raw compute_pins_fingerprint(), see that function's
                      docstring — at the exact point video_path/thumbnail_path/duration_s are
                      already assigned — a snapshot of the pins that built THIS video, for the
                      publish-time staleness gate below (see Key conventions)
...
engine/
  ...
  render/
    asset_sourcer.py  PexelsVideoSource + WikipediaImageSource (with license metadata fetch);
                      ...
                      the cut has zero bound pins. compute_pins_fingerprint_for_render(db, cut_id) -> str
                      wraps it and never returns None — a completed render with zero bound pins
                      (every beat black-framed) gets EMPTY_PINS_FINGERPRINT instead, so it stays
                      distinguishable from "never rendered" (see that constant's docstring — an
                      independent review caught a real gap where using the raw function at the
                      write/read sites let a black-frame cut's staleness gate be permanently, not
                      just temporarily, defeated). render.py (writer, at render success) and gate.py
                      (reader, at publish time) both use the _for_render wrapper, never the raw
                      function, so there is exactly one implementation of "how a render's
                      fingerprint is computed" — see Key conventions
  ...
  publish/
    ...
    gate.py           assert_safe_to_publish() / unsafe_assets() — enforces Asset.safe_to_publish
                      ...
                      compute_pins_fingerprint_for_render(db, cut.id) (never the raw
                      compute_pins_fingerprint() — see asset_sourcer.py's entry above); raises
                      ValueError on mismatch, no-ops when rendered_pins_fingerprint is None
                      (legacy/rollout-safety skip — see Key conventions); takes a Cut object,
                      deliberately asymmetric with assert_safe_to_publish's cut_id — publish_cut
                      is the sole caller of both and already has the Cut loaded
...
tests/
  ...
  test_render_task.py          14 tests — missing cut, already-posted cut refused, success clears stale error, music wiring, reel.tts_voice/text_color threaded into get_tts_provider()/composite_cut(), black_frame_beat_indices flagged when a beat resolves no real media and stays None when every beat does;
                              ...
                              exists here): compute_pins_fingerprint_for_render (not the raw compute_pins_fingerprint
                              — using the wrong one here would silently reopen the black-frame gap below) is
                              patched and asserted called with cut.id, its return value asserted on
                              cut.rendered_pins_fingerprint — the stronger "reflects real pins" property is
                              test_publish_gate.py's staleness end-to-end tests, not this file's
  ...
  test_asset_sourcer.py        14 tests — resolve_or_reuse pin, reuse, re-pin, beat isolation, commits (no open transaction on either resolve or reuse path), Wikipedia names all searched before any asset is flushed;
  ...
  test_publish_gate.py         8 tests — safe_to_publish enforcement; assert_video_matches_pins (Phase 7e) —
                              ...
                              (mutation-tested — the rollout-safety property design §7 depends on); a
                              successful all-black-frame render (rendered_pins_fingerprint = the
                              EMPTY_PINS_FINGERPRINT sentinel, not None) still catches a later partial
                              re-pin that fails before completing — the independent-review-caught gap
                              where a raw None here would have been indistinguishable from "never
                              rendered" and permanently, not just temporarily, exempted a cut whose asset
                              sourcing keeps failing; one end-to-end regression test reproducing the exact
                              bug sequence with real resolve_or_reuse() calls (mutation-verified against a
                              reverted fix)
